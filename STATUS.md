...
- 已通过：系统 Python 基线检查、WSL 资源与 GPU 可见性、项目环境导入、短求解烟雾测试、完整基线运行。
- 基线结果：完整 5 步实验成功；时间严格递增；电压范围 2.5–3.6 V；输出 manifest、时间序列、逐循环摘要、PNG 与日志。
- 待验证：环境删除/重建、SEI/析锂五循环、重复性、Notebook、Git 卫生、Linux CI、公开仓库。
...
1. 实现带严格参数溯源的 SEI/析锂方法示例。
2. 完成环境重建、重复性与 Notebook 验证。
3. 完成 CI、公开扫描和 GitHub 发布。
