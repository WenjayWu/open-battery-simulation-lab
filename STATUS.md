...
| 项目 | 状态 | 说明 |
| --- | --- | --- |
| Windows 主库 | 已确认 | 位于 `Simulation/Projects/open-battery-simulation-lab/`；外层 `Simulation/` 为非 Git 总工作区 |
...
- 工作区迁移：Git HEAD、origin 和历史保持不变；迁移前后原有结果均为 60 个文件、2,929,595 字节，聚合校验值一致；项目环境已在新路径重建。
...
- 当前阻塞：无。系统钥匙串内的 GitHub CLI 登录有效，但宿主进程中的失效 `GITHUB_TOKEN` 会优先覆盖钥匙串并造成 401；在干净终端中使用钥匙串登录或移除失效环境变量即可。Git 推送与公开 CI 均已验证，未读取或保存明文令牌。
...
- 2026-09-10：将仓库整体迁移到 `Simulation/Projects/open-battery-simulation-lab/`，仓库外建立 `Software/` 与 `Projects/` 分区；原 `COMSOL/` 更名为 `comsol-projects/`，软件本体预留位置为外层 `Software/COMSOL/`。
